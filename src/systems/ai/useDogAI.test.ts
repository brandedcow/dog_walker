...
describe('useDogAI', () => {
  ...
  it('moves toward player in COMING state and respects BOND attribute', () => {
    ...
    act(() => {
      result.current.update(delta, playerPos, DogState.COMING, setDogState, [], { strength: 1, focus: 1, agility: 1, bond: 1 });
    });
    ...
    act(() => {
      result.current.update(delta, playerPos, DogState.COMING, setDogState, [], { strength: 1, focus: 1, agility: 1, bond: 10 });
    });
