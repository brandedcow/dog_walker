...
export const TrainingOverlay = () => {
  const { playerStats, dogStats, setMenuState } = useGameStore();
  return (
    <OverlayContainer title="TRAINING" onBack={() => setMenuState('IDLE')}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div>
          <h4 style={{ margin: '0 0 10px 0' }}>PLAYER STRENGTH</h4>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            <div style={{ flex: 1, height: '10px', background: '#333', borderRadius: '5px', overflow: 'hidden' }}>
              <div style={{ width: `${(playerStats.strength / 10) * 100}%`, height: '100%', background: '#44ff44' }} />
            </div>
            <span>Lvl {playerStats.strength}</span>
          </div>
        </div>
        <div>
          <h4 style={{ margin: '0 0 10px 0' }}>DOG RECALL</h4>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            <div style={{ flex: 1, height: '10px', background: '#333', borderRadius: '5px', overflow: 'hidden' }}>
              <div style={{ width: `${(dogStats.recallSpeed / 20) * 100}%`, height: '100%', background: '#44ccff' }} />
            </div>
            <span>{dogStats.recallSpeed.toFixed(1)}</span>
          </div>
        </div>
        <p style={{ fontSize: '12px', textAlign: 'center', opacity: 0.5 }}>Grit required for upgrades: 100 (Current: {playerStats.grit})</p>
      </div>
    </OverlayContainer>
  );
};
